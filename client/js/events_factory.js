  ...
  methods: {
    ...
    clicked: function(event) {
      ...
      form.innerHTML =
        ...
        "" +
        `<button type='button' class='btn' onclick=\"${rsvp_f}('${event._id}'); closeForm2();\">${rsvp_msg}</button><button type='button' class='btn' onclick=\"${favorite_f}('${event._id}'); closeForm2();\">${favorite_msg}</button><button type="button" class="admin btn cancel" onclick="deleteEvent('${event._id}'); closeForm2()">(ADMIN) DELETE EVENT</button>` +
